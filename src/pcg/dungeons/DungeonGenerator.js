...
export class DungeonGenerator {
  ...
  static generate(map, minRoomSize) {
    ...
    let extraConnections = this.addNearbyConnections(
      rooms,
      connections,
      20,   // max distance (tune this!)
      0.2   // probability
    );

    connections =  connections.concat(extraConnections);
  ...
  // Adds additional connections to make overall dungeon look less tree-like
  // only connects already nearby rooms, so hallways don't cut into other rooms
  static addNearbyConnections(rooms, existingConnections, maxDist = 25, chance = 0.25) {
    let extra = [];
    ...
    for (let i = 0; i < rooms.length; i++) {
      for (let j = i + 1; j < rooms.length; j++) {
        ...
        let alreadyConnected = existingConnections.some(c =>
          (c.from === r1 && c.to === r2) ||
          (c.from === r2 && c.to === r1)
        );
        ...
        let dist = Math.abs(c1.x - c2.x) + Math.abs(c1.y - c2.y);

        // ensures rooms being connected are nearby
        if (dist <= maxDist && Math.random() < chance) {
          extra.push({ from: r1, to: r2 });
        }
